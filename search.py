...
class Search:
    def __init__(self,
        ...
        # Parameters
        self.board = board
        self.subgeno = subgeno
        self.bias_geno = bias_geno
        self.hnode = hnode
        self.net = net
    ...
    def qsearch(self, alpha: int, beta: int, ply: int) -> int:
        ...
        if ply >= MAX_PLY:
            if self.net == None: 
                return Eval.CoevEvoEvaluation.evaluate(self.board, self.subgeno, self.bias_geno, self.hnode)
            else:
                return Eval.EShyperNEATEvaluation.evaluate(self.board, self.net)

        # staticEval
        if self.net == None:
            bestValue = Eval.CoevEvoEvaluation.evaluate(self.board, self.subgeno, self.bias_geno, self.hnode)
        else:
            bestValue = Eval.EShyperNEATEvaluation.evaluate(self.board, self.net)
